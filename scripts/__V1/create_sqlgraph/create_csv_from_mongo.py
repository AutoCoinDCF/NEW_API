...
class Test():
    ...
    def create_collection(self, databse: str, collection: str, **kwargs):
        ...
        """
        collection_list = self.handler.client['LabeledEventGraph'].list_collection_names()
...
def run():
    test = Test()
    data_list = test.find("LabeledEventGraph", "eventData", {})
    ...
    for index, row in data_frame.iterrows():
        ...
        if row["event_attribute.event_attribute"] == "event_attribute":
            ...
            relation_list.append([relation_id1, row["entity_id"], "include_event",  "include_event", row["doc_id"]])
            relation_list.append([relation_id2, row["doc_id"], "include_event", "include_event", row["entity_id"]])
        # entity和 event 关系
        for entity in row["entity_list"]:
            if entity["id"].lower() != "none":
                ...
                relation_list.append([relation_id1, row["entity_id"], entity["role"], entity["role"], entity["id"]])
                relation_list.append([relation_id2, entity["id"], entity["role"], entity["role"], row["entity_id"]])
    ...
    for index, row in doc_frame.iterrows():
        entity_dict = []
        document_id = row["_id"]
        for sen in row["human_nel"]:
            for entity in sen:
                if entity["entity_id"] in entity_dict:
                    continue
                else:
                    entity_dict.append(entity["entity_id"])
                    relation_id1 = "{}-{}".format(document_id, entity["entity_id"])
                    relation_id2 = "{}-{}".format(entity["entity_id"], document_id)
                    relation_list.append([relation_id1, document_id,  "include_entity",  "include_entity", entity["entity_id"]])
                    relation_list.append([relation_id2, entity["entity_id"], "include_entity", "include_entity", document_id])

    relation_dataframe = pd.DataFrame(relation_list, columns=["id", "Head_id", "Relation_id", "Relation_name", "Tail"])
