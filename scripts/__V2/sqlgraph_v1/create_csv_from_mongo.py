...
def run():
    test = Test()
    data_list = test.find("QBData", "events", {})
    print("--------data_list----------")
    print(data_list)
    doc_list = test.find("QBData", "documents2", {})
    graph = Graph()
    ...
    for index, row in doc_frame.iterrows():
        ...
        for sen in row["entity_list"]:
            ...
            if sen["id"] in entity_dict:
                ...
                relation_list.append([relation_id1, document_id,  "include_entity",  "include_entity", sen["id"]])
                relation_list.append([relation_id2, sen["id"], "include_entity", "include_entity", document_id])

    relation_dataframe = pd.DataFrame(relation_list, columns=["id", "Head_id", "relation_id", "type", "Tail"])
